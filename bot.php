...
// ========== CONFIG ==========
$botToken = getenv("8647546201:AAHO7k3dazUN50np5NaT2pZdTMzJxPelOHE") ?: "";
...
// ========== DEFAULT SETTINGS ==========
$settings = json_decode(file_get_contents(__DIR__ . "/settings.json"), true); if(!is_array($settings)) $settings = [];
...
// ========== DEFAULT CATEGORIES ==========
$categories = json_decode(file_get_contents(__DIR__ . "/categories.json"), true); if(!is_array($categories)) $categories = [];
if(count($categories) == 0){
    ...
    file_put_contents(__DIR__ . "/categories.json", json_encode($categories));
}
...
// ========== LOAD DATA ==========
$balances = json_decode(file_get_contents(__DIR__ . "/balances.json"), true); if(!is_array($balances)) $balances = [];
$users = json_decode(file_get_contents(__DIR__ . "/users.json"), true); if(!is_array($users)) $users = [];
$orders = json_decode(file_get_contents(__DIR__ . "/orders.json"), true); if(!is_array($orders)) $orders = [];
$products = json_decode(file_get_contents(__DIR__ . "/products.json"), true); if(!is_array($products)) $products = [];
...
// ========== TEXT MESSAGES ==========
    ...
    $temp = json_decode(file_get_contents(__DIR__ . "/temp.json"), true);
    ...
    if($waiting == "newcat" && $user_id == $adminID){
        ...
        file_put_contents(__DIR__ . "/temp.json", json_encode($temp));
    ...
    // ---- ADD USER BALANCE - AMOUNT ----
        ...
        file_put_contents(__DIR__ . "/balances.json", json_encode($balances));
...
function buyPlan($chat_id, $message_id, $user_id, $pid, $plan_index){
    ...
    file_put_contents(__DIR__ . "/orders.json", json_encode($orders));
...
function telegramRequest($method, $data = []){
    ...
    if(empty($website)){
        error_log("Telegram bot token is missing. Set BOT_TOKEN in Render Environment Variables.");
        return false;
    }
...
function sendDocument($chat_id, $document, $caption, $reply_markup = null){
    ...
    if($reply_markup) $d['reply_markup'] = $reply_markup;
    return telegramRequest("sendDocument", $d);
}
...
function saveTemp($u, $k, $v){
    $t = json_decode(file_get_contents(__DIR__ . "/temp.json"), true);
    if(!is_array($t)) $t = [];
    if(!isset($t[$u]) || !is_array($t[$u])) $t[$u] = [];
    $t[$u][$k] = $v;
    file_put_contents(__DIR__ . "/temp.json", json_encode($t));
}
...
function sendMessage($c, $t, $r=null){
    ...
    return telegramRequest("sendMessage", $d);
}
...
function editMsg($c, $m, $t, $r=null){
    ...
    if($r) $d['reply_markup'] = $r;
    return telegramRequest("editMessageText", $d);
}
...
function sendPhoto($c, $p, $cap, $r=null){
    ...
    if($r) $d['reply_markup'] = $r;
    return telegramRequest("sendPhoto", $d);
}
